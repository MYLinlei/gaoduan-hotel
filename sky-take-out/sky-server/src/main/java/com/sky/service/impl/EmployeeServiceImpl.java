...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public PageResult pageQuery(EmployeePageQueryDTO employeePageQueryDTO) {
        ...
        //PageHelper需要在pom文件中配置坐标，传递页码和每页的pageSize。
        PageHelper.startPage(employeePageQueryDTO.getPage(),employeePageQueryDTO.getPageSize());
        //第二步调用Mapper层的sql语句，(sql语句查询的就是所以符合条件的记录)不需要传入参数页码和pageSize，并把返回值传给Page
        //Page类是的PageHelper的自定义类，封装了总查询数量和该页的数据集合，所以Page具有集合的属性
        Page<Employee> page = employeeMapper.pageQuery(employeePageQueryDTO);
        //或许总记录和和该页数据，然后再封装到PageResult
    ...
    public void editPassword(PasswordEditDTO passwordEditDTO) {
        ...
        updateEmployee.setId(currentId);
